...
class TestQuantizedTensor(TestCase):
    ...
    def _test_qtensor_masked_fill(self, device):
        ...
        ones = torch.ones(numel).to(torch.float).to(device)
        ...
        mask = torch.randint(0, 2, (numel, )).to(device)
        mask = mask.bool()
        x = torch.rand(numel).to(device)
        ...
        for qtype, fill_with in itertools.product(types, fills):
            ...
            for i in range(numel):
                if mask[i]:
                    ...
                    ref[i] = torch.Tensor([fill_with]).to(device)
